...
def parseOptions():
    ...
    parser.add_option(
        ...
        "--config",
        dest="config",
        default="rplidar.json",
        help="specify an rplidar.json file for this sensor",
        metavar="FILE",
    )
...
connected = False
print(f"Attempting to connect to RoboRio @{host_ip} \n \n")

while connected == False:
    result = os.system(f"ping -c 1 {host_ip} -q")

    if result == 0:
        # Check if the output contains "0% packet loss"
        connected = True
        print(f"\n \n Connected to NetworkTables @IP: {host_ip} \n \n")
        break
...
while not configured: 
    try: 
        ...
        print(f"Can't find lidar on {port}")
        continue
...
try:
    ...
    for each in rpl.iter_scans():
        ...
        select = (data[:, 2] != 0) & (data[:, 1] > roi_lower) & (data[:, 1] < roi_upper)
